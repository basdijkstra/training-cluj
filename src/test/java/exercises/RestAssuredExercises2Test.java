...
public class RestAssuredExercises2Test {
	...
	@DataProvider
	public static Object[][] countriesAndCircuits() {
		return new Object[][] {
				{"monza","Italy"},
				{"sepang","Malaysia"},
				{"spa", "Belgium"}
	...
	@Test
	@UseDataProvider("countriesAndCircuits")
	public void checkCountryForCircuit(String circuitName, String countryName) {
		
		given().
			spec(requestSpec).
				and().pathParam("nameOfMyCircuit",circuitName).
		when().
				get("/circuits/{nameOfMyCircuit}.json").
		then().
				assertThat().
				body("MRData.CircuitTable.Circuits.Location[0].country", equalTo(countryName));
	}
